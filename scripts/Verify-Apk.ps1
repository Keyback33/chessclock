...
$versionMatch = [regex]::Match((Get-Content (Join-Path $projectRoot 'pubspec.yaml') -Raw), '(?m)^version:\s*([0-9.]+)\+(\d+)')
if (!$versionMatch.Success) { throw 'Versión de distribución inválida.' }
$version = $versionMatch.Groups[1].Value
$versionCode = $versionMatch.Groups[2].Value
$apkName = "chessclock-$version.apk"
$apkPath = Join-Path $projectRoot "dist/$apkName"
$metadataPath = Join-Path $projectRoot "dist/$version"
...
$signature | Set-Content (Join-Path $metadataPath 'signature.txt')
...
if (!($badging -match "sdkVersion:'24'") -or !($badging -match "versionName='$([regex]::Escape($version))'") -or !($badging -match "versionCode='$versionCode'")) { throw 'Versión o mínimo Android inesperado.' }
$badging | Set-Content (Join-Path $metadataPath 'apk-info.txt')
$permissions | Set-Content (Join-Path $metadataPath 'permissions.txt')
$hash = (Get-FileHash -LiteralPath $apkPath -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  $apkName" | Set-Content (Join-Path $projectRoot "dist/$apkName.sha256")
